...
func controlLights(lightGrid map[Coord]struct{}, rule string, start Coord, end Coord) map[Coord]struct{} {
	for i := start.X; i <= end.X; i++ {
		for j := start.Y; j <= end.Y; j++ {
			switch strings.TrimSpace(rule) {
			case "turn on":
				lightGrid[Coord{X: i, Y: j}] = struct{}{}
			case "turn off":
				delete(lightGrid, Coord{X: i, Y: j})
			case "toggle":
				_, exists := lightGrid[Coord{X: i, Y: j}]
				if exists {
					delete(lightGrid, Coord{X: i, Y: j})
				} else {
					lightGrid[Coord{X: i, Y: j}] = struct{}{}
...
func main() {
	...
	lightGrid := make(map[Coord]struct{})

	for i := 0; i < len(input); i++ {
		lightGrid = controlLights(lightGrid, input[i].rule, input[i].start, input[i].end)
	}
	fmt.Println("Total amount of lights:", len(lightGrid))
}
